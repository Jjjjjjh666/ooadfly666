...
public class ExchangeCancelStrategy implements AftersaleCancelStrategy {
    ...
    public void cancel(AftersaleOrder order, String reason) {
        ...
        if (order.getExpressId() != null) {
            try {
                log.info("准备取消换货运单: expressId={}", order.getExpressId());
                ...
                log.info("换货运单取消成功: expressId={}", order.getExpressId());
            } catch (Exception e) {
                log.error("取消换货运单失败: expressId={}", order.getExpressId(), e);
                throw new RuntimeException("取消换货运单失败: " + e.getMessage(), e);
            }
        } else {
