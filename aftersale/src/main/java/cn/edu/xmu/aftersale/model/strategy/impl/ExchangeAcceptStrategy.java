...
/**
 * 换货验收策略：通过->已验收；不通过->已拒绝并生成返件运单
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class ExchangeAcceptStrategy implements AftersaleAcceptStrategy {

    private final LogisticsClient logisticsClient;

    @Override
    public void accept(AftersaleOrder order, Boolean accept, String conclusion) {
        log.info("执行换货验收策略: orderId={}, accept={}", order.getId(), accept);

        if (Boolean.TRUE.equals(accept)) {
            order.accept(conclusion != null ? conclusion : "验收通过");
            log.info("换货验收通过: orderId={}, 状态->已验收", order.getId());
        } else {
            ...
            log.info("换货验收不通过: orderId={}, expressId={}", order.getId(), expressId);
        }
    }
